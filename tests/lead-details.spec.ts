...
test("click on lead row, open lead details dialog and close it", async ({
  ...
  const { leadDetailsHeadingDialog, closeButtonDialog } = await clickOnLeadRow(
    page
  );
...
test("edit one lead and save changes", async ({ page }) => {
...
test("convert one lead to opportunity", async ({ page }) => {
  const { closeButtonDialog, leadDetailsHeadingDialog } = await clickOnLeadRow(
    page
  );
